...
> Doctrine for the `human-writer` skill, applied to marketing long-form: blog posts, README prose, landing pages, long-form newsletters. Loaded in addition to language-specific stylistic tells (`tells-stylistic-ar.md`).
...
4. **Em-dash overuse**: cap at 4 per 1000 words (EN) / 2 per 1000 words (FR) / target 0 (AR, "—" is foreign). Acceptable for rhetorical emphasis within cap.
...
- **A few em-dashes for emphasis**: within the cap (and for Arabic, target 0 — "—" is foreign to Arabic typography). A single emphatic dash in a 600-word EN/FR piece reads as voice, not AI.
...
```markdown
- [ ] Ran `scripts/analyze.py --type marketing --lang ar` — score ≤ 25
- [ ] First 100 words contain zero AI constructions and zero Tier-1 suspect vocabulary
- [ ] No conclusion-template close ("في الختام" / "في نهاية المطاف" / "خلاصة القول")
- [ ] No bullet list longer than 8 items, and not more than 3 bullet lists per 1000 words
- [ ] At least 1 specific opinion or take in the piece (not a hedged "يرى البعض أن")
- [ ] At least 1 concrete number, named entity, or date in the piece
- [ ] Sentence length variance (stdev) ≥ 8 (analyzer green)
- [ ] Em-dash count: target 0 (AR — "—" is foreign)
- [ ] Every `,` `;` `?` adjacent to Arabic is the Arabic ، ؛ ؟
- [ ] No emoji used as section header
- [ ] No "أبرز النقاط" / "ما ستتعلمه" block, no thesis-in-paragraph-1
```
...
```bash
python3 scripts/analyze.py --input draft.md --lang ar --type marketing --format human
```
...
### Example 2: Blog post opener (AR)

**Before (AI-flavored, ~70 words):**

<!-- human-writer:ignore-start (deliberately AI-flavored AR sample for teaching; must not self-flag) -->
> في عالم اليوم حيث البيانات هي الملكة، تجدر الإشارة إلى أهمية تسخير أدوات السحب لإطلاق العنان لإمكانات استراتيجيتك التجارية. سواء كنت شركة ناشئة أو مجموعة كبيرة، قد يبدو التنقل في منظومة بيانات الويب أمرا معقدا. وفي الواقع، الأمر يتعلق بفهم التحديات لاحتضان هذا التحول. دعونا ننغمس في قلب هذه الثورة الصامتة.

*Analyzer score: ~82 (CRITICAL). Hits: في عالم اليوم، تجدر الإشارة، تسخير، أطلق العنان لإمكانات، منظومة، التنقل، وفي الواقع، دعونا ننغمس؛ construction "سواء كنت … أو"؛ no number; cliché opener.*
<!-- human-writer:ignore-end -->

**After (humanized, ~70 words):**

<!-- human-writer:ignore-start (quoted AR model output sample for teaching) -->
> الاثنين الماضي، أرسل لي عميل ملف CSV من ١٢ ألف سطر. السكربت الذي ركّبناه في مارس كان لا يزال يعمل، لكن ٣٠٪ من الأسعار خاطئة: المصدر غيّر بنية HTML مساء أربعاء، دون إعلان. هذا المقال يحكي كيف رصدنا الانحراف في أربع ساعات بدل أربعة أسابيع، ولماذا «السكربتات التي تعمل» وهم إن لم تراقب مخرجاتها.

*Analyzer score: ~14 (LOW_RISK). Delta: −68. Concrete time anchor (الاثنين الماضي، مارس، مساء أربعاء)، numbers (١٢ ألف، ٣٠٪، ٤ ساعات vs ٤ أسابيع)، opinion («وهم»)، zero suspect vocab.*
<!-- human-writer:ignore-end -->
...
*Analyzer score: ~88 (CRITICAL). Hits: transform, seamlessly, unlock, robust, empowers, navigate (6), 1 AI construction (Whether you're), zero concrete claim.*
<!-- human-writer:ignore-end -->
...
- `tells-stylistic-ar.md`: full AR suspect vocabulary list and AI construction patterns
