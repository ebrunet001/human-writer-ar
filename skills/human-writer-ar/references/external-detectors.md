...
|---|---|---|---|---|
| **Copyleaks** | copyleaks.com/ai-content-detector | Pay-per-scan (credit packs) | Most mainstream in B2B / enterprise; what clients run against your deliverables | High false-positive rate on technical / translated / non-native text |
| **GPTZero** | gptzero.me | Subscription tiers (free + paid) | Educator-focused, transparent metrics (burstiness + perplexity), gives sentence-level breakdown | Lower accuracy on commercial / marketing content; weaker outside English |
...
Then open a new shell (or re-source your profile) and verify:
...
```bash
SCORE=$(python3 scripts/analyze.py --input draft.md --lang ar --type marketing \
...
- **Technical writing with formulaic structure.** RFCs, postmortems,
  and structured READMEs all trigger "AI-shaped" features regardless of
  authorship.
- **Non-native / translated text.** Translated and second-language writing
