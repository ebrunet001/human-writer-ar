...
> **RTL / Arabic-script note (human-writer-ar).** This file is copied verbatim from the master `human-writer`; the EN/FR examples are kept for cross-reference and the doctrine is language-agnostic. Two adjustments apply to Arabic:
> 1. **RTL is a display concern only.** The analyzer works on logical character order. Markdown bullets/headers (`-`, `##`) are written the same way regardless of script; `detect_bullet_parallelism` and `detect_header_pyramid` are unchanged and work on Arabic.
> 2. **List/heading conventions differ.** Arabic prose tolerates longer, more clause-chained sentences and uses the waw و to bind list items, so the "asymmetric bullets" and "vary H3 counts" advice still applies, but judge Arabic headings against Arabic norms (no Title Case concept; sentence-fragment headings are natural). The tricolon detector matches the Arabic comma-waw rhythm (`X، Y، وZ`); see `tells-stylistic-ar.md`.
