#!/usr/bin/env python3
"""
human-writer-ar / analyze.py — deterministic AI-tell detector (Arabic, MSA).

Specialization of the master human-writer analyzer for Modern Standard Arabic.
The text is processed in LOGICAL character order; RTL is a display concern only —
no bidi handling, no reversal. Differences vs the master (everything else is
identical):
  - --lang choices=["ar"].
  - count_sentences / sentence_lengths split on Arabic terminators too:
    [.؟!؛] (؟ = U+061F Arabic question mark, ؛ = U+061B Arabic semicolon).
  - detect_tricolons uses the Arabic comma ، (U+060C) + waw و (often attached to
    the next word, no space).
  - detect_suspect_vocab is CLITIC-AWARE: an Arabic stem also matches when
    preceded by attached clitics (و/ف/ب/ك/ل) and/or the article ال.
  - NEW detect_latin_punct_in_arabic: flags Latin , ; ? characters used adjacent
    to Arabic letters (where ، ؛ ؟ should be), wired as a low-weight branch.
  - Em-dash detector unchanged (counts U+2014 "—", foreign to Arabic).
...
def count_sentences(text: str) -> int:
    # Arabic adds its own terminators: ؟ (U+061F Arabic question mark) and
    # ؛ (U+061B Arabic semicolon), alongside the Latin . ! ?. We keep . ! ?
    # because Arabic text routinely mixes the Latin period (no Arabic full stop
    # exists) and AI output often emits Latin ? where ؟ belongs.
...
def build_recommendations(tells: dict) -> list[dict]:
    ...
    lp = styl.get("latin_punct_in_arabic")
    if lp and lp.get("flag"):
        recs.append({
            "priority": "medium" if lp.get("severity") == "high" else "low",
            "action": (
                f"Replace Latin punctuation used inside Arabic text: {lp.get('value')} "
                "occurrence(s) of , ; or ? sit next to Arabic letters. Use the Arabic "
                "comma ، (U+060C), semicolon ؛ (U+061B), and question mark ؟ (U+061F)."
            ),
        })
...
def compute_score(tells: dict, weights: dict) -> int:
    ...
    if tricolons >= 3:
        stylistic_pts += 5
    # NEW: Latin-punctuation-in-Arabic typography tell — SMALL low-weight branch
    # (Arabic). Latin , ; ? used where ، ؛ ؟ belong is a real AI surface signature
    # but is register-sensitive (technical Arabic and code-mixed copy legitimately
    # carry some Latin punctuation), so it contributes only a few points and never
    # dominates the score.
    lp = styl.get("latin_punct_in_arabic")
    if lp and lp.get("flag"):
        stylistic_pts += 5 if lp.get("severity") == "high" else 3
...
# Arabic tricolon: "X، Y، وZ" / "X، Y وZ". Arabic lists use the Arabic comma ،
# (U+060C) between items and the waw و before the last item. The waw is a clitic
# conjunction that ATTACHES to the following word with no space (وZ), so the
# pattern allows the optional comma before the waw and requires the waw to be
# glued to the closing word: …، و\w+. \w matches Arabic letters under Python's
# default Unicode flags. Calibrated to require the comma rhythm so it does not
# fire on every ordinary "X and Y" pair (which has no leading comma list).
TRICOLON_AR = re.compile(r"\w+\s*،\s*\w+\s*،?\s*و\w+")


def detect_tricolons(text: str) -> int:
    return len(TRICOLON_AR.findall(text))


# Arabic letter range (U+0600–U+06FF covers the core Arabic block; the
# Presentation Forms blocks are excluded because well-formed modern text does not
# use them). A Latin , ; or ? is a TELL only when it is used where an Arabic
# ، ؛ ؟ should be — i.e. immediately adjacent to Arabic script. A Latin question
# mark after an English acronym, or a comma inside a bracketed Latin citation,
# is not the tell, so we require Arabic-letter adjacency on either side.
_ARABIC_LETTER = r"[؀-ۿ]"
# For each Latin punctuation char, match it only when an Arabic letter sits
# directly before OR after it (allowing one optional space on the Arabic side,
# the way the misuse actually appears: "النص, " or " ?النص").
_LATIN_PUNCT_IN_ARABIC_RX = re.compile(
    r"(?:" + _ARABIC_LETTER + r"\s?[,;?])"
    r"|(?:[,;?]\s?" + _ARABIC_LETTER + r")"
)


def detect_latin_punct_in_arabic(text: str, threshold: int) -> dict:
    """Arabic typography tell: Latin , ; ? used where ، ؛ ؟ belong.

    Native Arabic uses the Arabic comma ، (U+060C), Arabic semicolon ؛ (U+061B),
    and Arabic question mark ؟ (U+061F). AI plain-text output frequently emits the
    Latin , ; ? instead (English habit). We count every Latin , ; or ? that is
    adjacent to an Arabic letter (i.e. used inside Arabic text), and flag when the
    count exceeds `threshold`. The detector is low-weight and calibrated NOT to
    fire on clean native prose (which uses ، ؛ ؟ throughout). Latin punctuation
    that sits among Latin letters (acronyms, URLs, code) is NOT counted because it
    requires Arabic-letter adjacency.
    """
    count = len(_LATIN_PUNCT_IN_ARABIC_RX.findall(text))
...
# Boundaries between Arabic tokens: whitespace and Arabic/Latin punctuation. We
# do NOT use \b because Arabic clitics (و/ف/ب/ك/ل + ال) glue to the stem with no
# word break, so a bare stem like "كلمة" appears as "الكلمة" / "وكلمة" with no \b
# between the prefix and the stem.
_AR_TOKEN_BOUNDARY = r"[\s،؛؟.!?:()«»\"'\-]"


def detect_suspect_vocab(text: str, vocab: list[str]) -> list[dict]:
    """Clitic-aware Arabic vocabulary matcher.

    Vocabulary entries are listed as BARE stems. Arabic attaches several
    single-letter clitic prefixes (و = and, ف = then, ب = with/by, ك = like,
    ل = for/to) and the definite article ال directly to the stem with no space.
    So the stem "كلمة" must also match "الكلمة", "وكلمة", "والكلمة", "بالكلمة",
    etc. We therefore allow an OPTIONAL leading clitic letter [وفبكل] and an
    OPTIONAL article ال before the escaped stem, anchored on token boundaries
    (start-of-text or an Arabic/Latin separator) so we don't match mid-word.
    """
    out = []
    prefix = r"(?:^|(?<=" + _AR_TOKEN_BOUNDARY + r"))(?:[وفبكل])?(?:ال)?"
    suffix = r"(?=$|" + _AR_TOKEN_BOUNDARY + r")"
    for word in vocab:
        pat = re.compile(prefix + re.escape(word) + suffix, flags=re.IGNORECASE)
...
def analyze(text: str, lang: str, content_type: str) -> dict:
    ...
    latin_punct = detect_latin_punct_in_arabic(text, thresholds["latin_punct_in_arabic_max"])
    ...
    tells = {
        ...
        "stylistic": {
            ...
            "latin_punct_in_arabic": latin_punct,
        },
        "structural": {
...
def render_human(result: dict) -> str:
    lines: list[str] = []
    lines.append("=" * 60)
    lines.append(f"  human-writer-ar / analyze.py — {result.get('input_file', '<stdin>')}")
    lines.append("=" * 60)
    lines.append(
    ...
    lp = styl.get("latin_punct_in_arabic") or {}
    lp_flag = "FLAG" if lp.get("flag") else "ok"
    lines.append(
        f"latin punct  : {lp.get('value', 0)} Latin , ; ? adjacent to Arabic "
        f"(should be ، ؛ ؟)  [{lp_flag}]"
    )
...
def main() -> int:
    p = argparse.ArgumentParser(description="Deterministic AI-tell analyzer (Arabic, MSA)")
    p.add_argument("--input", type=Path, help="Input file (markdown/txt). Stdin if omitted.")
    p.add_argument("--lang", choices=["ar"], required=True)
